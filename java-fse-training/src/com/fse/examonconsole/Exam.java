...
public class Exam {
	...
	public static void main(String[] args) {
		...
		Question question6 = new Question("1", "What is the extension of compiled java classes?", ".jv", ".java",
				".jcf", ".class", 4);
		Question question7 = new Question("1", "Which of the following is a superclass of every class in Java?",
				"Object Class", "String Class", "Java Class", "Java Lang Class", 1);
		Question question8 = new Question("1", "Which one of the following is not an access modifier?", "private",
				"protected", "public", "volatile", 4);
		...
		Question question10 = new Question("1", "What is a map in Java?", "Data structure",
				"Defined in java.util package", "Represented using key-value pairs", "All of the above", 4);
		...
		logger.info("Welcome to Exam On Console");
		logger.info("Are you ready (Y-Proceed,N-Stop)");
		@SuppressWarnings("resource")
		Scanner scaninput = new Scanner(System.in);
		String readyDecision = scaninput.next();
		if (readyDecision.equalsIgnoreCase("N")) {
			logger.warn("User not ready for test - Program Terminates");
			return;
		}
		for (Question eachquestion : questionArray) {
			logger.info(eachquestion.toString());
			int userAnswer = scaninput.nextInt();
			if (userAnswer != eachquestion.getCorrectOption()) {
				logger.warn("Your answer is not correct. Do you want to try again? (Y-Yes,N-No)");
				...
				if (repeatQuestionCheck.equalsIgnoreCase("Y")) {
					...
					if (user2ndAnswer != eachquestion.getCorrectOption()) {
						logger.error("Your answer is not correct again");
